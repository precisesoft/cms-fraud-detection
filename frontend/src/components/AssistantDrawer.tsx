...
export function AssistantDrawer({ isOpen, onClose, context }: AssistantDrawerProps) {
  const [input, setInput] = React.useState('');
  const [messages, setMessages] = React.useState<Array<{ role: 'user' | 'assistant'; content: string; data?: ChatResponse }>>([]);
  ...
  const handleSend = async (prompt?: string) => {
    ...
    try {
      ...
      const assistantMsg = { role: 'assistant' as const, content: response.answer, data: response };
          ...
          >
            <div className="flex h-full flex-col">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h2 className="font-bold text-slate-900">Investigation Assistant</h2>
                    ...
                    <div className="mt-4 rounded-xl border border-indigo-100 bg-indigo-50 px-4 py-3 text-xs text-indigo-900">
                      Powered by text-to-SQL. Ask questions about providers, claims, risk scores, and fairness.
                    </div>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                {!messages.length && (
                  <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center">
                    ...
                    <h3 className="mt-4 text-base font-bold text-slate-900">Ask about this {context.type}</h3>
                    <p className="mt-2 text-sm text-slate-500">
                      Questions are answered via text-to-SQL against the backend database.
                    </p>
                  </div>
                )}
                    ...
                    >
                      ...
                      {msg.data?.sql && (
                        <div className="mt-3 rounded-lg bg-slate-50 border border-slate-200 p-3">
                          <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">SQL</p>
                          <code className="text-xs text-slate-600 whitespace-pre-wrap break-all">{msg.data.sql}</code>
                        </div>
                      )}
                      {msg.data?.rows && msg.data.rows.length > 0 && msg.data.columns.length > 0 && (
                ...
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Suggested Prompts</p>
              ...
              <div className="border-t border-slate-100 px-6 py-4 space-y-3">
                {messages.length > 0 && (
                  <button onClick={handleClear} className="text-xs font-semibold text-slate-500 hover:text-slate-700">
                    Clear Thread
                  </button>
                )}
